...
class ArrayConfig(BaseModel):
    ...
    model_path: str = Field(description="joblib model file path.", title="Model path")
    metadata_path: str = Field(description="JSON metadata file path.", title="Metadata path")
    exclude_limiting_entity_ids: list[str] = Field(
        ...
        title="Exclude limiting entities",
    )
...
class HyperparamConfig(BaseModel):
    ...
    n_estimators: list[int] = Field(default=[200], title="n_estimators")
    max_depth: list[int] = Field(default=[5], title="max_depth")
    learning_rate: list[float] = Field(default=[0.08], title="learning_rate")
    subsample: list[float] = Field(default=[0.9], title="subsample")
    min_child_weight: list[int] = Field(default=[1], title="min_child_weight")
...
class HaDiscoveryConfig(_HelperHaDiscoveryConfig):
    ...
    device_name: str = Field(default="MIMIRHEIM PV Learner", title="Device name")
...
class PvLearnerConfig(BaseModel):
    ...
    ha_discovery: HaDiscoveryConfig = Field(default_factory=HaDiscoveryConfig, title="HA discovery")
