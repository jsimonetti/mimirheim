"""Tests for PvOpenMeteoDaemon's Config Service protocol wiring.

Covers only the wiring between PvOpenMeteoDaemon and ConfigOwnerSupport
...
from pv_openmeteo.__main__ import CONFIG_OWNER_ID, PvOpenMeteoDaemon
...
def _make_daemon(tmp_path: Path) -> PvOpenMeteoDaemon:
    config = PvOpenMeteoConfig.model_validate(
        {
            ...
            "arrays": {
                "solaredge": {
                    "planes": [{"declination": 25, "azimuth": -90, "peak_power_kwp": 1.04}]
                },
            },
        }
    )
    return PvOpenMeteoDaemon(config, tmp_path / "config.yaml")
...
class TestOnConnect:
    def test_publishes_the_descriptor_and_subscribes_alongside_the_trigger_topic(
        ...
        assert "mimir/input/tools/pv/trigger" in subscribed
