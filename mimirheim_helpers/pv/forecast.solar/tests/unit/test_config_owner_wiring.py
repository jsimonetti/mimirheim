"""Tests for PvFetcherDaemon's Config Service protocol wiring.

Covers only the wiring between PvFetcherDaemon and ConfigOwnerSupport
...
from pv_fetcher.__main__ import CONFIG_OWNER_ID, PvFetcherDaemon
from pv_fetcher.config import ArrayConfig, MqttConfig, PvFetcherConfig
...
def _make_daemon(tmp_path: Path) -> PvFetcherDaemon:
    config = PvFetcherConfig(
    ...
    return PvFetcherDaemon(config, tmp_path / "config.yaml")
