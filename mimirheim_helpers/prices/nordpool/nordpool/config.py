...
class NordpoolApiConfig(BaseModel):
    ...
    Attributes:
        ...
        price_interval: Length of one published price step —
            ...
            hour: the four quarter-hour spot prices are then averaged into one
            hourly spot price before the formulas are applied, which is exactly
            how such suppliers derive their hourly rate. Leaving it at
            ``"quarter_hourly"`` on an hourly contract makes the optimiser
            chase intra-hour price swings the meter never bills for.
    """
...
class NordpoolConfig(BaseModel):
    ...
    output_topic: str | None = Field(
        ...
        title="Output topic", json_schema_extra={"ui_placeholder": "{mimir_topic_prefix}/input/prices"},
    )
    ...
    signal_mimir: bool = Field(default=False, description="Publish to mimir_trigger_topic after publishing prices.", title="Signal mimirheim")
    mimir_trigger_topic: str | None = Field(default=None, description="Topic to trigger mimirheim. Derived from mimir_topic_prefix when not set.", title="mimirheim trigger topic")
