"""Tests for ZonneplanPricesDaemon's Config Service protocol wiring.

Covers only the wiring between ZonneplanPricesDaemon and ConfigOwnerSupport
...
from zonneplan_prices.__main__ import CONFIG_OWNER_ID, ZonneplanPricesDaemon
from zonneplan_prices.config import MqttConfig, ZonneplanApiConfig, ZonneplanPricesConfig
...
def _make_daemon(tmp_path: Path) -> ZonneplanPricesDaemon:
    config = ZonneplanPricesConfig(
        mqtt=MqttConfig(host="localhost", client_id="test"),
        trigger_topic="mimir/input/tools/prices/trigger",
        zonneplan=ZonneplanApiConfig(email="user@example.com"),
    )
    return ZonneplanPricesDaemon(config, tmp_path / "config.yaml")
