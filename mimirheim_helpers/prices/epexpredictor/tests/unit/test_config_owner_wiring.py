"""Tests for EpexPredictorPricesDaemon's Config Service protocol wiring.

Covers only the wiring between EpexPredictorPricesDaemon and
...
that wiring does not disturb the daemon's existing trigger-handling behavior
(HelperDaemon's own subscribe/discovery/debounce logic), only adds to it.
"""
...
from epexpredictor_prices.__main__ import CONFIG_OWNER_ID, EpexPredictorPricesDaemon
from epexpredictor_prices.config import EpexPredictorApiConfig, EpexPredictorPricesConfig, MqttConfig
...
def _make_daemon(tmp_path: Path) -> EpexPredictorPricesDaemon:
    ...
    return EpexPredictorPricesDaemon(config, tmp_path / "config.yaml")
