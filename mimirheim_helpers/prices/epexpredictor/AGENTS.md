# epexpredictor_prices — Agent Instructions
...
uv run pytest mimirheim_helpers/prices/epexpredictor/tests
uv run ruff check .                           # must be clean before a change is done
uv run python -m epexpredictor_prices --config config.yaml
```

The module path is `epexpredictor_prices`, not a dotted path under
...
- [wiki/Helpers/EpexPredictor.md](../../../wiki/Helpers/EpexPredictor.md) — setup guide, confidence decay design, pairing with a day-ahead source.
...
mimirheim_helpers/prices/epexpredictor/
  ...
  epexpredictor_prices/
    ...
    config.py           # Pydantic config schema (EpexPredictorPricesConfig, etc.)
    fetcher.py           # HTTP GET from the EpexPredictor API; returns FetchResult
...
Its default `output_topic` is the same canonical topic Nordpool defaults to.
When running both helpers together, one of the two must set `output_topic`
explicitly so they land on different entries of mimirheim's `inputs.prices`
list.
