...
def to_jedison_object_schema(model_schema: dict[str, Any]) -> dict[str, Any]:
    ...
    then resolves the one concern that cannot be decided from a single
    field in isolation: this editor's grouping hint (`x-mimir-group`). Each
    field carrying that hint gets Jedison's `x-category` key set to the
    hint's value, and if any field in the schema carries the hint, this
    returned top-level object schema gets `x-format` set once, to
    ...
    Returns:
        A new schema dict (the input, and its `properties` sub-dicts, are
        not mutated). `properties` is replaced with the per-field mapped
        ...
        no `x-format` key at all, matching Jedison's own fallback behaviour
        for an ungrouped object schema.
    """
