...
class EntityConfig(BaseModel):
    ...
    unit: Literal["W", "kW"] = Field(description="Power unit reported by this entity.")
...
class HaConfig(BaseModel):
    ...
    url: str = Field(description="Base URL of the HA instance including scheme and port.")
    token: str = Field(description="Long-Lived Access Token generated in HA.")
    sum_entities: list[EntityConfig] = Field(min_length=1)
    subtract_entities: list[EntityConfig] = Field(default_factory=list)
    lookback_days: int = Field(default=7, ge=1, le=112)
    lookback_decay: float = Field(default=1.0, ge=1.0)
...
class BaseloadConfig(BaseModel):
    ...
    trigger_topic: str | None = Field(
        default=None,
        description="MQTT topic that triggers a fetch cycle. Defaults to '{mimir_topic_prefix}/input/tools/baseload/trigger'."
    )
    ...
    homeassistant: HaConfig = Field(description="HA connection and entity configuration.")
