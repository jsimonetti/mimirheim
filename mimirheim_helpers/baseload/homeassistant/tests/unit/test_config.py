...
Covers schema validation for all Pydantic models used by the homeassistant baseload tool.
"""
...
from baseload_ha.config import EntityConfig, HaConfig, MqttConfig, BaseloadConfig
...
class TestEntityConfig:
    ...
    def test_invalid_unit_rejected(self) -> None:
        with pytest.raises(ValidationError):
            EntityConfig(entity_id="sensor.power", unit="MW")
...
class TestHaConfig:
    ...
    def test_subtract_entities_defaults_to_empty_list(self) -> None:
        cfg = HaConfig(
            url="http://ha.local", token="t", sum_entities=[self._entity()]
        )
        assert cfg.subtract_entities == []
    ...
    def test_sum_entities_must_not_be_empty(self) -> None:
        with pytest.raises(ValidationError):
            HaConfig(url="http://ha.local", token="t", sum_entities=[])
    ...
    def test_entity_invalid_unit_rejected(self) -> None:
        with pytest.raises(ValidationError):
            HaConfig(
                url="http://ha.local",
                token="t",
                sum_entities=[{"entity_id": "sensor.p1", "unit": "MW"}],
            )
