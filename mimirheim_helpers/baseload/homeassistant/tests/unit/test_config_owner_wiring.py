"""Tests for HaBaseloadDaemon's Config Service protocol wiring.

Covers only the wiring between HaBaseloadDaemon and ConfigOwnerSupport
...
from baseload_ha.__main__ import CONFIG_OWNER_ID, HaBaseloadDaemon
from baseload_ha.config import BaseloadConfig, EntityConfig, HaConfig, MqttConfig
...
def _make_daemon(tmp_path: Path) -> HaBaseloadDaemon:
    config = BaseloadConfig(
        ...
        homeassistant=HaConfig(
            ...
            token="secret-token",
            sum_entities=[EntityConfig(entity_id="sensor.kitchen_power", unit="W")],
        ),
    )
    return HaBaseloadDaemon(config, tmp_path / "config.yaml")
