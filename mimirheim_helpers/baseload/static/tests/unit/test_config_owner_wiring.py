"""Tests for StaticBaseloadDaemon's Config Service protocol wiring.

Covers only the wiring between StaticBaseloadDaemon and ConfigOwnerSupport
...
prove that wiring does not disturb the daemon's existing trigger-handling
...
from baseload_static.__main__ import CONFIG_OWNER_ID, StaticBaseloadDaemon
from baseload_static.config import BaseloadConfig, MqttConfig, StaticBaseloadConfig
...
def _make_daemon(tmp_path: Path) -> StaticBaseloadDaemon:
    config = BaseloadConfig(
        mqtt=MqttConfig(host="localhost", client_id="test"),
        trigger_topic="mimir/input/tools/baseload/trigger",
        baseload=StaticBaseloadConfig(profile_kw=[1.0] * 24),
    )
    return StaticBaseloadDaemon(config, tmp_path / "config.yaml")
...
class TestOnConnect:
    def test_publishes_the_descriptor_and_subscribes_alongside_the_trigger_topic(
        ...
        assert "mimir/input/tools/baseload/trigger" in subscribed
...
class TestOnMessage:
    ...
    def test_other_messages_still_reach_the_normal_dispatch(self, tmp_path: Path) -> None:
        ...
        # dispatch (which ignores unrelated messages).
        daemon._on_message(MagicMock(), None, message)
