...
Kept as a separate, colocated artifact from ``baseload_ha_db.config`` per
...
``baseload_ha_db.__main__``) combines with ``BaseloadConfig`` to build its
...
for those fields rather than re-authoring their labels. ``baseload_ha_db``
nests this module's own ``EntityConfig`` and ``HaConfig``, so their FormSpecs
are authored here. Per ADR-0006,
...
ENTITY_CONFIG_FORM_SPEC = FormSpec(
    fields={
        ...
        "unit": FieldSpec(
            label="Unit override",
            description=(
                "Unit override. When unset, this is auto-detected from statistics_meta."
            ),
            tier=Tier.EXPERT,
            option_labels={
                "W": "Watts (W)",
        ...
        "outlier_factor": FieldSpec(
            label="Outlier factor",
            description="Threshold multiplier for P99-based outlier detection.",
            tier=Tier.EXPERT,
        ),
    }
)
...
HA_CONFIG_FORM_SPEC = FormSpec(
    fields={
        "db_url": FieldSpec(
            label="DB URL",
            description="SQLAlchemy database URL pointing to the HA recorder database.",
            tier=Tier.BASIC,
        ),
...
BASELOAD_CONFIG_FORM_SPEC = FormSpec(
    fields={
        "homeassistant": FieldSpec(
            label="Home Assistant DB",
            description="HA recorder database connection and entity configuration.",
            tab="Home Assistant DB",
            tier=Tier.BASIC,
