"""Tests for HaDbBaseloadDaemon's Config Service protocol wiring.

Covers only the wiring between HaDbBaseloadDaemon and ConfigOwnerSupport
...
from baseload_ha_db.__main__ import CONFIG_OWNER_ID, HaDbBaseloadDaemon
from baseload_ha_db.config import BaseloadConfig, EntityConfig, HaConfig, MqttConfig
...
def _make_daemon(tmp_path: Path) -> HaDbBaseloadDaemon:
    config = BaseloadConfig(
        ...
        homeassistant=HaConfig(
            ...
            sum_entities=[EntityConfig(entity_id="sensor.kitchen_power")],
        ),
    )
    return HaDbBaseloadDaemon(config, tmp_path / "config.yaml")
