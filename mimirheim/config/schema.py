...
class MqttConfig(BaseModel):
    ...
    host: str = Field(description="MQTT broker hostname or IP address.", title="Broker host")
    port: int = Field(default=1883, description="MQTT broker port.", title="Broker port")
    client_id: str | None = Field(default=None, description="MQTT client identifier. Defaults to 'mimir' when not set.", title="Client ID")
    topic_prefix: str = Field(default="mimir", description="Topic prefix for all mimirheim topics.", title="Topic prefix")
    username: str | None = Field(default=None, description="Broker username. Omit for anonymous access.", title="Username")
...
class GridConfig(BaseModel):
    ...
    import_limit_kw: float = Field(ge=0, description="Maximum grid import power in kW.", title="Import limit (kW)")
    export_limit_kw: float = Field(ge=0, description="Maximum grid export power in kW.", title="Export limit (kW)")
...
class BalancedWeightsConfig(BaseModel):
    ...
    cost_weight: float = Field(ge=0, default=1.0, description="Weight on cost/revenue terms.", title="Cost weight")
    self_sufficiency_weight: float = Field(
        ge=0, default=1.0, description="Weight on grid import penalty terms.", title="Self-sufficiency weight"
    )
...
class ObjectivesConfig(BaseModel):
    ...
    balanced_weights: BalancedWeightsConfig | None = Field(
        ...
        title="Balanced strategy weights",
    )
...
class ConstraintsConfig(BaseModel):
    ...
    max_import_kw: float | None = Field(default=None, description="Hard cap on grid import in kW.", title="Max import cap (kW)")
    max_export_kw: float | None = Field(default=None, description="Hard cap on grid export in kW.", title="Max export cap (kW)")
...
class ControlConfig(BaseModel):
    ...
    min_enforcer_dwell_steps: int = Field(
        ...
        title="Min enforcer dwell steps",
    )
...
class InputsConfig(BaseModel):
    ...
    prices: list[str] = Field(
        ...
        title="Prices topics", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/prices"},
    )
...
class OutputsConfig(BaseModel):
    ...
    schedule: str | None = Field(
        ...
        title="Schedule topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/strategy/schedule"},
    )
    current: str | None = Field(
        ...
        title="Current topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/strategy/current"},
    )
    last_solve: str | None = Field(
        ...
        title="Last solve topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/status/last_solve"},
    )
    availability: str | None = Field(
        ...
        title="Availability topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/status/availability"},
    )
...
class HomeAssistantConfig(BaseModel):
    ...
    enabled: bool = Field(default=False, description="Enable HA MQTT discovery. Default: false.", title="Enable HA discovery")
...
class DebugConfig(BaseModel):
    ...
    enabled: bool = Field(default=False, description="Enable DEBUG logging and solve dumps.", title="Enable debug dumps")
    dump_dir: Path | None = Field(
        default=None,
        description="Directory for solve dumps. Null = disabled.",
        title="Dump directory",
    )
    max_dumps: int = Field(ge=0, default=50, description="Maximum retained dump pairs.", title="Max debug dumps")
...
class ReportingConfig(BaseModel):
    ...
    notify_topic: str | None = Field(
        ...
        title="Notify topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/status/dump_available"},
    )
...
class EfficiencySegment(BaseModel):
    ...
    power_max_kw: float = Field(gt=0, description="Maximum power through this segment in kW.", title="Max power (kW)")
    efficiency: float = Field(gt=0, le=1.0, description="Round-trip efficiency fraction [0, 1].", title="Efficiency")
...
class EfficiencyBreakpoint(BaseModel):
    ...
    power_kw: float = Field(ge=0.0, description="AC power in kW at this breakpoint.", title="Power (kW)")
    efficiency: float = Field(gt=0.0, le=1.0, description="Round-trip efficiency fraction.", title="Efficiency")
...
class BatteryOutputsConfig(BaseModel):
    ...
    exchange_mode: str | None = Field(
        ...
        title="Exchange mode topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/battery/{name}/exchange_mode"},
    )
    soc_ratchet: str | None = Field(
        ...
        title="SOC ratchet status topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/status/battery/{name}/soc_ratchet"},
    )
...
class SocRatchetConfig(BaseModel):
    ...
    The mechanism is a ratchet with a deadline and a hold. A dynamic minimum SOC
    climbs by a step for every interval that passes without a measured full
    charge, up to a cap; once the due time falls inside the horizon the SOC is
    constrained to reach the target and to stay there. Three design choices,
    each because this is a planner with a horizon rather than a controller
    reacting to live SOC:
    ...
    - The charge is **held** at the top, and it is planned to ``target_pct``
      rather than to the reset threshold. Passive balancing bleeds the highest
      cells at tens of milliamps, and only while they sit in the upper voltage
      knee; the BMS recalibrates its SOC estimate only once the charge current
      has tapered at the voltage limit. Both need dwell time at the top. That
      dwell does not happen by itself when a planner dictates the SOC
      trajectory, so the plan asks for it explicitly: ``soc[t] >= target`` on
      enough consecutive step boundaries to cover ``hold_hours``. A single
      quarter-hour at 97% gives neither, which is why the target is 100% and
      the reset threshold is not.
    ...
    enters the solve horizon, the SOC is constrained to reach ``target_pct``
    and stay there for ``hold_hours``. That is a constraint on the state at a
    time, not an instruction to charge at a time: the solver still chooses
    which quarter-hours to buy the energy in, which is the whole reason for
    doing this inside the optimiser instead of leaving it to the inverter.
    ...
    Attributes:
        ...
        target_pct: The SOC, as a percentage of ``capacity_kwh``, the plan is
            asked to reach and hold. Default 100: only at the top of the
            configured capacity do the cells sit in the voltage knee where the
            balancer works; lower it if the reported SOC cannot reach 100.
            Must be at least
            ``full_threshold_pct``; a plan that aims below what counts as
            success could never reset the policy. The two are deliberately
            separate numbers: this is what to plan for, ``full_threshold_pct``
            is what a BMS reading has to show for the plan to have worked.
        hold_hours: How long the SOC must stay at or above ``target_pct`` once
            it gets there, in hours. Default 2.0: a passive balancer bleeds at
            tens of milliamps, so what it can correct scales with dwell time,
            and the charge current needs time of the same order to taper at the
            voltage limit. Both are pack-specific; two hours is the order of
            magnitude a charger's absorption stage typically runs, so set this
            from the pack's documentation where it gives a figure. Rounded up
            to whole steps in the plan. Applies to the
            observation as well: the policy resets only after a reading has
            been at or above ``full_threshold_pct`` for this long without
            dipping, so a plan that touched the top and fell away does not
            count. 0 restores a single-step touch, planned and observed.
    """
    ...
    hold_hours: float = Field(
        ...
        title="Hold at target (hours)",
    )

    @model_validator(mode="after")
    def _check_target_bounds(self) -> "SocRatchetConfig":
        """Reject a plan target the reset can never see.

        The plan charges to ``target_pct``; the policy resets when a reading
        shows ``full_threshold_pct``. If the first is below the second, every
        balance charge completes exactly as planned and the policy never learns
        it did: the floor keeps climbing and the deadline is re-imposed at step
        0 forever. Equal is fine and is the pre-hold behaviour.
        """
        if self.target_pct < self.full_threshold_pct:
            raise ValueError(
                f"target_pct ({self.target_pct}) must be at least full_threshold_pct "
                f"({self.full_threshold_pct}); a plan that aims below what counts as "
                "a full charge can never reset the policy."
            )
        return self
...
class BatteryConfig(BaseModel):
    ...
    **SOS2 piecewise-linear model** (``charge_efficiency_curve``): Uses a SOS2
    ...
    model_config = ConfigDict(extra="forbid",
        json_schema_extra={
            "description": "Configuration for a DC-coupled residential battery.",
        },
    )
    ...
    discharge_efficiency_curve: list[EfficiencyBreakpoint] | None = Field(
        ...
        title="Discharge efficiency curve",
    )
    ...
    optimal_lower_soc_kwh: float = Field(
        ...
        title="Preferred minimum SOC (kWh)",
    )
    ...
    reduce_charge_above_soc_kwh: float | None = Field(
        ...
        title="Derate charge above SOC (kWh)",
    )
    reduce_charge_min_kw: float | None = Field(
        ...
        title="Derated charge minimum (kW)",
    )
    reduce_discharge_below_soc_kwh: float | None = Field(
        ...
        title="Derate discharge below SOC (kWh)",
    )
    reduce_discharge_min_kw: float | None = Field(
        ...
        title="Derated discharge minimum (kW)",
    )
    ...
    min_discharge_kw: float | None = Field(
        ...
        title="Minimum discharge power (kW)",
    )
...
class EvInputsConfig(BaseModel):
    ...
    plugged_in_topic: str | None = Field(
        ...
        title="Plug state topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/ev/{name}/plugged_in"},
    )
...
class EvOutputsConfig(BaseModel):
    ...
    exchange_mode: str | None = Field(
        ...
        title="Exchange mode topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/ev/{name}/exchange_mode"},
    )
    loadbalance_cmd: str | None = Field(
        ...
        title="Load balance topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/ev/{name}/loadbalance"},
    )
...
class EvConfig(BaseModel):
    ...
    min_charge_kw: float | None = Field(
        ...
        title="Minimum EVSE charge power (kW)",
    )
    min_discharge_kw: float | None = Field(
        default=None,
        ge=0.0,
        ...
        title="Minimum V2H discharge power (kW)",
    )
...
class PvOutputsConfig(BaseModel):
    ...
    power_limit_kw: str | None = Field(
        ...
        title="Power limit topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/pv/{name}/power_limit_kw"},
    )
    zero_export_mode: str | None = Field(
        ...
        title="Zero-export mode topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/pv/{name}/zero_export_mode"},
    )
    on_off_mode: str | None = Field(
        ...
        title="On/off mode topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/pv/{name}/on_off_mode"},
    )
    is_curtailed: str | None = Field(
        ...
        title="Is curtailed topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/pv/{name}/is_curtailed"},
    )
...
class PvConfig(BaseModel):
    ...
    **Staged** (``production_stages``): the inverter only accepts discrete power
    ...
    topic_forecast: str | None = Field(
        ...
        title="Forecast topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/pv/{name}/forecast"},
    )
...
class HybridInverterOutputsConfig(BaseModel):
    ...
    exchange_mode: str | None = Field(
        ...
        title="Exchange mode topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/output/hybrid/{name}/exchange_mode"},
    )
...
class HybridInverterConfig(BaseModel):
    ...
    min_soc_kwh: float = Field(ge=0, default=0.0, description="Minimum SOC in kWh.", title="Minimum SOC (kWh)")
    max_charge_kw: float = Field(gt=0, description="Maximum DC charge power to battery cells in kW.", title="Max charge power (kW)")
    max_discharge_kw: float = Field(
        gt=0, description="Maximum DC discharge power from battery cells in kW.",
        title="Max discharge power (kW)",
    )
    ...
    topic_pv_forecast: str | None = Field(
        ...
        title="PV forecast topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/hybrid/{name}/pv_forecast"},
    )
    inputs: BatteryInputsConfig | None = Field(
        default_factory=BatteryInputsConfig,
...
class DeferrableLoadConfig(BaseModel):
    ...
    power_profile: list[float] = Field(
        ...
        title="Power profile (kW per step)",
    )
    topic_window_earliest: str | None = Field(
        ...
        title="Earliest start topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/deferrable/{name}/window_earliest"},
    )
    topic_window_latest: str | None = Field(
        ...
        title="Latest end topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/deferrable/{name}/window_latest"},
    )
    topic_committed_start_time: str | None = Field(
        ...
        title="Committed start time topic",
    )
    topic_recommended_start_time: str | None = Field(
        ...
        title="Recommended start time topic",
    )
...
class StaticLoadConfig(BaseModel):
    ...
    topic_forecast: str | None = Field(
        ...
        title="Forecast topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/baseload/{name}/forecast"},
    )
...
class BuildingThermalInputsConfig(BaseModel):
    ...
    topic_current_indoor_temp_c: str | None = Field(
        ...
        title="Indoor temperature topic",
    )
    topic_outdoor_temp_forecast_c: str | None = Field(
        ...
        title="Outdoor temperature forecast topic",
    )
...
class ThermalBoilerInputsConfig(BaseModel):
    ...
    topic_current_temp: str | None = Field(
        ...
        title="Current temperature topic", json_schema_extra={"ui_placeholder": "{mqtt.topic_prefix}/input/thermal_boiler/{name}/temp_c"},
    )
...
class ThermalBoilerConfig(BaseModel):
    ...
    setpoint_c: float = Field(description="Target hot water temperature in °C.", title="Target temperature (°C)")
...
class HeatingStage(BaseModel):
    ...
    elec_kw: float = Field(ge=0.0, description="Electrical power at this stage in kW.", title="Electrical power (kW)")
    cop: float = Field(ge=0.0, description="COP at this stage.", title="COP")
...
class SpaceHeatingInputsConfig(BaseModel):
    ...
    topic_heat_produced_today_kwh: str | None = Field(
        ...
        title="Heat produced today topic",
    )
...
class CombiHeatPumpConfig(BaseModel):
    ...
    cop_sh: float = Field(gt=0, description="COP in space heating mode.", title="COP (space heating mode)")
    volume_liters: float = Field(gt=0, description="DHW tank volume in litres.", title="DHW tank volume (L)")
    setpoint_c: float = Field(description="DHW target temperature in °C.", title="DHW target temperature (°C)")
    min_temp_c: float = Field(default=40.0, description="DHW minimum temperature in °C.", title="DHW minimum temperature (°C)")
    cooling_rate_k_per_hour: float = Field(ge=0, description="Tank cooling rate in K/h.", title="Cooling rate (K/h)")
    min_run_steps: int = Field(ge=0, default=4, description="Minimum consecutive active steps.", title="Minimum run steps")
    wear_cost_eur_per_kwh: float = Field(
        ge=0,
        default=0.0,
        description=(
...
class MimirheimConfig(BaseModel):
    ...
    model_config = ConfigDict(
        extra="forbid",
        json_schema_extra={
            "title": "Mimirheim Configuration",
            "description": "Configuration for the Mimirheim Daemon.",
    ...
    batteries: dict[str, BatteryConfig] = Field(
        ...
        title="Batteries",
        json_schema_extra={
             "x-addPropertyContent": "Add battery",
             "x-mimir-adapter": "dict-key-as-title",
    ...
    pv_arrays: dict[str, PvConfig] = Field(
        ...
        title="PV arrays",
        json_schema_extra={
            "x-addPropertyContent": "Add PV array",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    ev_chargers: dict[str, EvConfig] = Field(
        ...
        title="EV chargers",
        json_schema_extra={
            "x-addPropertyContent": "Add EV charger",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    deferrable_loads: dict[str, DeferrableLoadConfig] = Field(
        ...
        json_schema_extra={
            "x-addPropertyContent": "Add deferrable load",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    static_loads: dict[str, StaticLoadConfig] = Field(
        ...
        title="Static loads",
        json_schema_extra={
            "x-addPropertyContent": "Add static load",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    hybrid_inverters: dict[str, HybridInverterConfig] = Field(
        ...
        json_schema_extra={
            "x-addPropertyContent": "Add hybrid inverter",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    thermal_boilers: dict[str, ThermalBoilerConfig] = Field(
        ...
        json_schema_extra={
            "x-addPropertyContent": "Add thermal boiler",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    space_heating_hps: dict[str, SpaceHeatingConfig] = Field(
        ...
        json_schema_extra={
            "x-addPropertyContent": "Add space heating heat pump",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    combi_heat_pumps: dict[str, CombiHeatPumpConfig] = Field(
        ...
        json_schema_extra={
            "x-addPropertyContent": "Add combi heat pump",
            "x-mimir-adapter": "dict-key-as-title",
        },
    )
    grid: GridConfig = Field(title="Grid connection")
    objectives: ObjectivesConfig = Field(default_factory=ObjectivesConfig, title="Objectives")
    constraints: ConstraintsConfig = Field(default_factory=ConstraintsConfig, title="Constraints")
    solver: SolverConfig = Field(default_factory=SolverConfig, title="Solver")
    readiness: ReadinessConfig = Field(default_factory=ReadinessConfig, title="Readiness")
    mqtt: MqttConfig = Field(title="MQTT")
    outputs: OutputsConfig = Field(default_factory=OutputsConfig, title="Output topics")
    inputs: InputsConfig = Field(default_factory=InputsConfig, title="Input topics")
    homeassistant: HomeAssistantConfig = Field(default_factory=HomeAssistantConfig, title="Home Assistant")
    debug: DebugConfig = Field(default_factory=DebugConfig, title="Debug")
    control: ControlConfig = Field(default_factory=ControlConfig, title="Control")
    reporting: ReportingConfig = Field(default_factory=ReportingConfig, title="Reporting")
